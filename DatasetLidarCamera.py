...
class DatasetLidarCameraKittiOdometry(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, use_reflectance=False,
        ...
        if split == 'val' or split == 'test':
            ...
            # print(len(self.val_RT),len(self.all_files))
    ...
    def __getitem__(self, idx):
        ...
        #pc.shape (125472, 4) pc.shape (107821, 4)
        pc_org = torch.from_numpy(pc.astype(np.float32)) # pc_org (N, 4)
        # print('pc_org.shape',pc_org.shape)
        # pc_org.shape torch.Size([125472, 4]) print('pc_org.shape',pc_org.shape)
        #表示选择所有行的第四列，即反射率列。
        reflectance=[]
        ...
        if self.split == 'test':
            sample = {'rgb': img, 'point_cloud': pc_in, 'calib': calib,
                      ...
                      'initial_RT': initial_RT,'reflectance':reflectance}
        else:
            sample = {'rgb': img, 'point_cloud': pc_in, 'calib': calib,
