...
from models.DCLCNet import DCLCNet
...
ex = Experiment("DCLCNet-evaluate-iterative")
...
def config():
    dataset = 'kitti/odom'
    data_folder = '/home/xinxi320/ourwork/zhao/DCLCNet/data'
    # data_folder = '/home/xinxi320/ourwork/zhao/data'
    test_sequence = 0
    ...
    output = '../output/000'#15为改进后评估保存的地方，多范围。15_1为多帧评估.多范围。15_2为复现的多帧评估
    save_image = False
    outlier_filter = True 
...
# weights = [
#    '/home/xinxi320/ourwork/zhao/DCLCNet/pretrained/kitti_iter1.tar',
#    '/home/xinxi320/ourwork/zhao/DCLCNet/pretrained/kitti_iter2.tar',
#    '/home/xinxi320/ourwork/zhao/DCLCNet/pretrained/kitti_iter3.tar',
#    '/home/xinxi320/ourwork/zhao/DCLCNet/pretrained/kitti_iter4.tar',
#    '/home/xinxi320/ourwork/zhao/DCLCNet/pretrained/kitti_iter5.tar',
# ]
weights = [
   '/home/xinxi320/ourwork/zhao/DCLCNet/checkpoints/kitti/odom/val_seq_18/models/checkpoint_r20.00_t1.50_e112_0.191.tar',
   '/home/xinxi320/ourwork/zhao/DCLCNet/checkpoints/kitti/odom/val_seq_19/models/checkpoint_r10.00_t1.00_e125_0.062.tar',
   '/home/xinxi320/ourwork/zhao/DCLCNet/checkpoints/kitti/odom/val_seq_19/models/checkpoint_r5.00_t0.50_e169_0.042.tar',
   '/home/xinxi320/ourwork/zhao/DCLCNet/checkpoints/kitti/odom/val_seq_19/models/checkpoint_r2.00_t0.20_e217_0.028.tar',
   '/home/xinxi320/ourwork/zhao/DCLCNet/checkpoints/kitti/odom/val_seq_19/models/checkpoint_r1.00_t0.10_e262_0.019.tar',
]
...
def lidar_project_depth_reflectance(pc_rotated,reflectance, cam_calib, img_shape):
    ...
    reflectance = reflectance.reshape(-1, 1)#变二维数组
    # reflectance = reflectance.reshape(pcl_uv.shape[0], -1)
    
    reflectance_img = np.zeros((img_shape[0], img_shape[1], 1))########单通道
    # reflectance_img[pcl_uv[:, 1], pcl_uv[:, 0]] = reflectance.squeeze()
    reflectance_img[pcl_uv[:, 1], pcl_uv[:, 0]] = reflectance
    reflectance_img = torch.from_numpy(reflectance_img.astype(np.float32))
    reflectance_img = reflectance_img.cuda()
    reflectance_img = reflectance_img.permute(2, 0, 1)#改变顺序
    pc_valid = pc_rotated.T[mask]
    return depth_img, pcl_uv, reflectance_img , pc_valid
...
def main(_config, seed):
    ...
    for i in range(len(weights)):
        ...
        if _config['network'].startswith('Res'):
            ...
            model = DCLCNet(input_size, use_feat_from=feat, md=md,
    ...
    ref_path = os.path.join(_config['output'], 'ref')
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        for idx in range(len(sample['rgb'])):
            ...
            pc_lidar = sample['point_cloud'][idx].clone()#旋转到相机坐标的点云
            ...
            if _config['use_reflectance']:
                ...
                reflectance = sample['reflectance'][idx].clone()
                depth_gt, uv_gt, reflectance_gt , pc_gt_valid=lidar_project_depth_reflectance(pc_lidar, reflectance, sample['calib'][idx], real_shape)
            ...
            pc_rotated = rotate_back(sample['point_cloud'][idx], RT_inv)  # Pc` = RT * Pc这时候的pc_rotated应该是旋转到相机坐标系下但是加了扰动的

            if _config['max_depth'] < 80.:
                pc_rotated = pc_rotated[:, pc_rotated[0, :] < _config['max_depth']].clone()

            if _config['use_reflectance']:
                depth_img, uv_input,reflectance_img , pc_input_valid=lidar_project_depth_reflectance(pc_rotated, reflectance, sample['calib'][idx], real_shape)
                depth_img /= _config['max_depth']
                reflectance_img/=0.8
            else:
                #depth_img应该是有扰动的深度图，uv_input=在图像上的位置，pc_valid=pc_rotated的转置
            ...
            shape_pad = [0, 0, 0, 0] #用于展示用，填充rbg达到某个尺寸
            ...
            if _config['use_reflectance']:
                reflectance_gt = F.pad(reflectance_gt, shape_pad)
                reflectance_img = F.pad(reflectance_img, shape_pad)
            
                ref_input.append(reflectance_img)
                ref_gt.append(reflectance_gt)
                ref_rotated_input.append(reflectance)
        ...
        if _config['save_ref']:
            reflectance_img = reflectance_img.detach().cpu().numpy()
            reflectance_img = (reflectance_img / np.max(reflectance_img)) * 255
            cv2.imwrite(os.path.join(ref_path, sample['rgb_name'][0]), reflectance_img[0, :376, :1241])
        ...
        if show:
            ...
            cv2.waitKey(1)
        if _config['use_reflectance']:
            ref_input = torch.stack(ref_input)
            ref_resize = F.interpolate(ref_input, size=[256, 512], mode="bilinear")
            
            mul_input= torch.cat((lidar_resize, ref_resize), dim=1)
        ...
        #到这里，我们在评估的时候，应该是要求出扰动的大小，越靠近这个值（mis_calib）就越准确。或者说预测的值和mis_calib之间就是评估的误差
        ...
        T_composed = RT1[:3, 3] #提取初始相机姿势中的平移部分
        R_composed = quaternion_from_matrix(RT1) #从初始相机姿势中提取旋转部分，并将其转换为四元数表示

        errors_t[0].append(T_composed.norm().item()) #计算并保存初始相机姿势的平移部分的范数
...
#  计算并保存初始相机姿势的欧拉角表示的旋转误差，表示为 errors_rpy[0]。这里通过 mat2xyzrpy 将旋转矩阵转换为欧拉角表示，并将弧度转换为角度。       
        rpy_error = mat2xyzrpy(RT1)[3:]
        rpy_error *= (180.0 / 3.141592)
        errors_rpy[0].append(rpy_error)

        log_string += [str(errors_t[0][-1]), str(errors_r[0][-1]), 
        ...
        with torch.no_grad():
            for iteration in range(start, len(weights)):
                ...
                if _config['iterative_method'] == 'single_range' or _config['iterative_method'] == 'single':
                    if _config['use_reflectance']:
                        T_predicted, R_predicted = models[0](rgb_resize, mul_input)
                    else:
                        T_predicted, R_predicted = models[0](rgb_resize, lidar_resize)
                elif _config['iterative_method'] == 'multi_range':
                    if _config['use_reflectance']:
                        T_predicted, R_predicted = models[iteration](rgb_resize, mul_input)
                    else:
                ...
                if _config['use_reflectance']:
                    if iteration == 0:
                        ...
                        rotated_ref =  ref_rotated_input[0]
                    ...
                    # rotated_ref = rotate_forward(rotated_ref, RT_predicted) # H_pred*X_init
                    
                    
                    depth_img_pred, uv_pred, reflectance_img_pred,pc_pred_valid=lidar_project_depth_reflectance(rotated_point_cloud, rotated_ref,sample['calib'][0], real_shape_input[0])
                    
                    depth_img_pred /= _config['max_depth']
                    reflectance_img_pred/=0.8
                    ...
                    ref_pred = F.pad(reflectance_img_pred, shape_pad_input[0])
                    ref = ref_pred.unsqueeze(0)
                    ref_resize= F.interpolate(ref, size=[256, 512], mode="bilinear")
    ...
    if _config['save_log']:
        log_file.close()
    print("Iterative refinement: ")##############这边开始输出
    for i in range(len(weights) + 1):
