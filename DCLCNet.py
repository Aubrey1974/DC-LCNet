...
from models.add.transformer import FeatureTransformer
from models.add.matching import global_correlation_softmax
from models.add.cost import CostVolume
from models.add.feature import (StereoNetFeature,FeaturePyrmaid,FeaturePyramidNetwork)
...
class DCLCNet(nn.Module):
    ...
    def __init__(self, image_size, use_feat_from=1, md=4, use_reflectance=False, dropout=0.0,
        ...
        """
        super(DCLCNet, self).__init__()
        input_lidar = 1
        self.res_num = res_num
        self.use_feat_from = use_feat_from

        self.use_StereoNetFeature=False
        ...
        if self.use_StereoNetFeature:
            self.corr_stere = CostVolume(81,feature_similarity='correlation')
        else:
        ...
        if self.use_StereoNetFeature:
            # self.stere =nn.Linear(275072, 512)
            self.stere =nn.Linear(fc_size * 4, 512)
    ...
    def forward(self, rgb, lidar):
        ...
        #c16 ([16, 512, 8, 16])  c26 ([16, 512, 8, 16])      


        c16, c26 = self.transformer(feature0=c16, feature1=c26)
        
        if self.use_StereoNetFeature:

            corr6=self.corr_stere(c16,c26)           
        else:
        ...
        # torch.Size([16, 529, 8, 16])



        # 这一行代码将输入 x 改变形状，将其视为一维张量。
        # 这通常是为了将卷积层输出的多维特征图拉平成一维向量，以便用于全连接层。
        x = x.view(x.shape[0], -1)  
        #Dropout 是一种正则化技术，通过在训练过程中随机将一部分神经元的输出置零，有助于减少过拟合
        x = self.dropout(x)

        if self.use_StereoNetFeature:
            x = self.leakyRELU(self.stere(x))


        # 将一维向量 x 传递到全连接层 self.fc1，然后应用 Leaky ReLU 激活函数。
        # Leaky ReLU 是修正线性单元（ReLU）的变体，通过允许小于零的值有一个小的斜率，避免了某些神经元“死亡”的问题。
        else:
            x = self.leakyRELU(self.fc1(x))
        # 类似地，对 x 进行另一组全连接层和 Leaky ReLU 操作，产生平移（translation）的输出。
