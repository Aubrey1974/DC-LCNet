...
from models.DCLCNet import DC-LCNet
...
def config():
    ...
    optimizer = 'adam'
    resume = True
    #weights = '/home/xinxi320/ourwork/zhao/DCLCNet/pretrained/kitti_iter5.tar'
    weights = '/home/xinxi320/ourwork/zhao/DCLCNet/checkpoints/kitti/odom/val_seq_19/models/checkpoint_r2.00_t0.20_e217_0.028.tar'
    #weights = None
    rescale_rot = 1.0
    rescale_transl = 2.0
    precision = "O0"
...
def lidar_project_depth_reflectance(pc_rotated,reflectance, cam_calib, img_shape):
    ...
    return depth_img, pcl_uv, reflectance_img
    


def get_reflectance_image(pc_rotated, reflectance, cam_calib, img_shape):
    pc_rotated = pc_rotated[:3, :].detach().cpu().numpy()
    ...
    #print("Shape of reflectance:", reflectance.shape)
    ...
    reflectance_img = reflectance_img.cuda()
    reflectance_img = reflectance_img.permute(2, 0, 1)#改变顺序
    #print("Shape of reflectance_img:", reflectance_img.shape)
...
def main(_config, _run, seed):
    ...
    train_iter = 0
    val_iter = 0
    print(starting_epoch,_config['epochs'])
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            for idx in range(len(sample['rgb'])):
                ...
                if _config['use_reflectance']:
                    ...
                    depth_gt, uv,reflectance_gt=lidar_project_depth_reflectance(pc_lidar, reflectance, sample['calib'][idx], real_shape)
                ...
                if _config['use_reflectance']:
                    depth_img, uv,reflectance_img=lidar_project_depth_reflectance(pc_rotated, reflectance, sample['calib'][idx], real_shape)
            ...
            if _config['use_reflectance']:
                ...
                ref_input = F.interpolate(ref_input, size=[256, 512], mode="bilinear")
                mul_input= torch.cat((lidar_input, ref_input), dim=1)
                loss, R_predicted,  T_predicted = train(model, optimizer, rgb_input, mul_input,
        ...
        for batch_idx, sample in enumerate(ValImgLoader):
            ...
            if _config['use_reflectance']:
                ...
                ref_input = F.interpolate(ref_input, size=[256, 512], mode="bilinear")
                mul_input= torch.cat((lidar_input, ref_input), dim=1)
                loss, trasl_e, rot_e, R_predicted,  T_predicted = val(model, rgb_input, mul_input,
                                                                    sample['tr_error'], sample['rot_error'],
                                                                    loss_fn, sample['point_cloud'], _config['loss'])
            else:
